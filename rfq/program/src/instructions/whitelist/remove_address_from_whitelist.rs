...
pub fn remove_address_from_whitelist_instruction(ctx:Context<RemoveAddressToWhitelistAccounts>,address:Pubkey) -> Result<()>{
    ...
    let index = whitelist_account.whitelist.iter().position(|x| x.to_string() == address.to_string()).unwrap();
...
fn validate_remove_address_inputs(whitelist: &Whitelist, address: &Pubkey) -> Result<()>{
    ...
    require!(whitelist.whitelist.len() > 0,
