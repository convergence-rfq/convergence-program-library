import { AnchorProvider, BN, parseIdlErrors, Program, ProgramError, Wallet, web3 } from "@project-serum/anchor";
import NodeWallet from "@project-serum/anchor/dist/cjs/nodewallet";
import { bs58 } from "@project-serum/anchor/dist/cjs/utils/bytes";
import { EuroPrimitive, IDL } from "./euro_primitive";
import { EuroMeta, ExpirationData, OptionType } from "./types";

export const MAINNET_FEE_OWNER_KEY = new web3.PublicKey("CyDnoEMVuf21v23bxoS2wXxPdCvRR2yFLfymegMH1WY4");

export const DEVNET_FEE_OWNER_KEY = new web3.PublicKey("Bja7SLji7JzzS5fwg2qUESoPbktALCMuUJLd8VCy8DkG");
 ...
 */
export const getClusterNameFromConnection = async (connection: web3.Connection): Promise<SolCluster> => {
...
export const CONTRACT_DECIMALS_BN = new BN(Math.pow(10, CONTRACT_TOKEN_DECIMALS));

export const contractAmountToUnderlyingAmount = (contractAmount: BN, underlyingAmountPerContract: BN) =>
  contractAmount.mul(underlyingAmountPerContract).div(CONTRACT_DECIMALS_BN);

export const contractAmountToStableAmount = (contractAmount: BN, euroMeta: EuroMeta) => {
  const underlyingAmount = contractAmount.mul(euroMeta.underlyingAmountPerContract).div(CONTRACT_DECIMALS_BN);
  const underlyingDecimalsFactor = TEN_BN.pow(new BN(euroMeta.underlyingDecimals));
...
export const calcValueInStables = (euroMeta: EuroMeta, expirationData: ExpirationData, amountOfContracts: BN): BN => {
 ...
 * @returns
 */
export const loadUnsetExpirationAccounts = (program: Program<EuroPrimitive>) => {
 ...
 * @param programId
 */
export const createProgram = (payer: web3.Keypair, jsonRpcUrl: string, programId: web3.PublicKey) => {
 ...
 * @param programId
 */
export const createProgramFromWallet = (wallet: Wallet, jsonRpcUrl: string, programId: web3.PublicKey) => {
  const connection = new web3.Connection(jsonRpcUrl);
 ...
 * @returns
 */
export const createProgramFromProvider = (provider: AnchorProvider, programId: web3.PublicKey) => {
  // Create anchor Program
  return new Program<EuroPrimitive>(IDL, programId, provider);
};
