...
describe("Psyoptions American instrument integration tests", async () => {
  ...
  it("create an RFq to buy 1 option ...", async () => {
    ...
    const sig = await options.mintPsyOPtions(context.maker, new anchor.BN(1), OptionType.CALL, context);
  ...
  it("Create a RFQ where taker wants to sell 2 options", async () => {
    ...
    const sig = await options.mintPsyOPtions(context.taker, new anchor.BN(2), OptionType.CALL, context);
    console.log("Option has been minted successfully with Signature:", sig);
    const tokenMeasurer = await TokenChangeMeasurer.takeSnapshot(
      context,
