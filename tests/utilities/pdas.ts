...
export async function getRfqPda(
  ...
  const quoteAssetDataSerialized = program.coder.types.encode(
    "QuoteAsset",
    quoteAsset.toQuoteData()
  );
  const hashedQuoteAsset = sha256(quoteAssetDataSerialized);
  const fixedSizeSerialized = program.coder.types.encode(
    "FixedSize",
    fixedSize
  );
  const [pda] = await PublicKey.findProgramAddress(
...
export async function getMintInfoPda(
  mintAddress: PublicKey,
  programId: PublicKey
) {
  const [pda] = await PublicKey.findProgramAddress(
    [Buffer.from(MINT_INFO_SEED), mintAddress.toBuffer()],
    programId
  );
  return pda;
}

export async function getCollateralTokenPda(
  user: PublicKey,
  programId: PublicKey
) {
  const [pda] = await PublicKey.findProgramAddress(
    [Buffer.from(COLLATERAL_TOKEN_SEED), user.toBuffer()],
    programId
  );
  return pda;
}

export async function getCollateralInfoPda(
  user: PublicKey,
  programId: PublicKey
) {
  const [pda] = await PublicKey.findProgramAddress(
    [Buffer.from(COLLATERAL_SEED), user.toBuffer()],
    programId
  );
  return pda;
}

export async function getQuoteEscrowPda(
  response: PublicKey,
  programId: PublicKey
) {
  const [pda] = await PublicKey.findProgramAddress(
    [Buffer.from(QUOTE_ESCROW_SEED), response.toBuffer()],
    programId
  );
  return pda;
}
...
export async function getInstrumentEscrowPda(
  ...
  const [pda] = await PublicKey.findProgramAddress(
    [
      Buffer.from(INSTRUMENT_ESCROW_SEED),
      response.toBuffer(),
      assetIdentifierToSeedBytes(assetIdentifier),
    ],
    programId
  );
  return pda;
...
export async function getPsyoptionsAmericanEscrowPda(
  response: PublicKey,
  legIndex: number,
  programId: PublicKey
) {
...
export async function getRiskEngineConfig(programId: PublicKey) {
  const [pda] = await PublicKey.findProgramAddress(
    [Buffer.from(RISK_ENGINE_CONFIG_SEED)],
    programId
  );
  return pda;
}
